...
Lets edit the main controller to redirect user to our todo route instead of displaying ``hello world``, and we add logout route to log user out and clear his session
...
<div class="container">
    ...
    <form action="/register" method="POST">
        ...
        <button type="submit">Register</button> <a class="btn back" href="/login">Already have an account? Login</a>
    </form>
</div>
</body>
</html>
...
<div class="container">
    ...
    <form action="/login" method="POST">
        ...
        <button type="submit">Login</button> <a class="btn back" href="/register">Don't have an account? Register</a>
    </form>
</div>
</body>
</html>
```
### Remark
When we set a session, we should be careful because it is accessible only in the controller where it is set and its sub-routes. For example, in our application, we set the session in the auth controller and set its path to `'/'`. This makes the session accessible throughout the entire application. However, if we set it path to `'auth'`, it will be accessible only in `'auth'` and `'auth/something'`. We will not be able to access it in `'todo/'` or `'user/'`.