...
```js
const [results, count] = await this.taskRepository.findAndCount({where: { user }, skip: offset,take: limit,order: { created_at: 'DESC' },});
```
...
```js
const [results, count] = await this.taskRepository.findAndCount({ where: { user }, skip, take: limit, order: { created_at: 'DESC' }, });
``` 
