...
```js
const [results, count] = await this.taskRepository.findAndCount({skip: offset,take: limit,order: { created_at: 'DESC' },});
```
...
```js
const [results, count] = await this.taskRepository.findAndCount({ skip, take: limit, order: { created_at: 'DESC' }, });
``` 
